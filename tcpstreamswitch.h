...
public:
    ...
    bool operator< (const TcpStreamId& id) const
    {
        if (src_addr < id.src_addr)
            return true;
        else if (src_addr > id.src_addr)
            return false;

        if (dst_addr < id.dst_addr)
            return true;
        else if (dst_addr > id.dst_addr)
            return false;

        if (src_port < id.src_port)
            return true;
        else if (src_port > id.src_port)
            return false;

        if (dst_port < id.dst_port)
            return true;
        else if (dst_port > id.dst_port)
            return false;
...
private:
    ...
    map<TcpStreamId, TcpStreamDataHandler*> _data_handlers;
};
